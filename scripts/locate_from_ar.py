...
def cleanFiles():
    ...
    img_dirs = os.listdir(testdata_path+"/imgs")
    if len(xml_dirs) > 0:
        for f in xml_dirs:
            print("remove:", f)
            os.remove(testdata_path+'/xmls/'+f)
    else:
        print("no pre files")
    if len(img_dirs) > 0:
        for f in img_dirs:
            print("remove:", f)
            os.remove(testdata_path+'/imgs/'+f)
    else:
        print("no pre files")
...
if __name__ == "__main__":
    ...
    camera_matrix = np.loadtxt(data_path + "/camera_info/cameraMatrix.csv", delimiter=",")
    distCoeffs = np.loadtxt(
        data_path + "/camera_info/distCoeffs.csv", delimiter=",")
